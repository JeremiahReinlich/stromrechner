...
class StromrechnerApp {
    ...
    zeigeTabelle() {
        ...
        // Lazy Loading: Nur MAX_EINTRAEGE_ANZEIGEN Einträge anzeigen
        const anzuzeigendeEintraege = interpolierteDaten.slice(0, this.MAX_EINTRAEGE_ANZEIGEN);

        let aktuellerMonat = '';
        anzuzeigendeEintraege.forEach(eintrag => {
            const eintragMonat = eintrag.datum.substring(0, 7); // YYYY-MM

            // Zusammenfassungszeile am Monatswechsel einfügen
            if (eintragMonat !== aktuellerMonat && aktuellerMonat !== '') {
                const zusammenfassung = monatsZusammenfassungen.find(z => z.monat === aktuellerMonat);
                if (zusammenfassung) {
            ...
            aktuellerMonat = eintragMonat;
        });

        // Zusammenfassungszeile für den letzten Monat hinzufügen
        if (aktuellerMonat !== '') {
            const letzteZusammenfassung = monatsZusammenfassungen.find(z => z.monat === aktuellerMonat);
            if (letzteZusammenfassung) {
                const zusammenfassungRow = this.erstelleMonatZusammenfassungZeile(letzteZusammenfassung);
                tbody.appendChild(zusammenfassungRow);
            }
        }

        // "Mehr laden"-Button hinzufügen, wenn mehr Einträge vorhanden sind
        if (interpolierteDaten.length > this.MAX_EINTRAEGE_ANZEIGEN) {
            const mehrLadenRow = document.createElement('tr');
            const mehrLadenCell = document.createElement('td');
            mehrLadenCell.colSpan = 5;
            mehrLadenCell.style.textAlign = 'center';
            mehrLadenCell.style.padding = '1rem';

            const mehrLadenBtn = document.createElement('button');
            mehrLadenBtn.textContent = `Mehr laden (${interpolierteDaten.length - this.MAX_EINTRAEGE_ANZEIGEN} weitere Einträge)`;
            mehrLadenBtn.className = 'btn-secondary';
            mehrLadenBtn.style.width = '100%';
            mehrLadenBtn.onclick = () => this.zeigeAlleEintraege(interpolierteDaten, monatsZusammenfassungen);

            mehrLadenCell.appendChild(mehrLadenBtn);
            mehrLadenRow.appendChild(mehrLadenCell);
            tbody.appendChild(mehrLadenRow);
        }
    }

    // Alle Einträge anzeigen (wenn "Mehr laden" geklickt wird)
    zeigeAlleEintraege(interpolierteDaten, monatsZusammenfassungen) {
        const tbody = document.getElementById('tabelle-body');
        // Letzte Zeile (Mehr laden-Button) entfernen
        tbody.removeChild(tbody.lastChild);

        // Monatliche Zusammenfassungen berechnen, falls nicht übergeben
        if (!monatsZusammenfassungen) {
            monatsZusammenfassungen = this.berechneMonatlicheZusammenfassung(interpolierteDaten);
        }

        let aktuellerMonat = '';
        interpolierteDaten.slice(this.MAX_EINTRAEGE_ANZEIGEN).forEach(eintrag => {
            const eintragMonat = eintrag.datum.substring(0, 7); // YYYY-MM
    ...
    berechneMonatlicheZusammenfassung(interpolierteDaten) {
        ...
        Object.values(monatsDaten).forEach(monat => {
            ...
            // HT/NT kWh gesamt berechnen
            const htKwhGesamt = interpolierteDaten
                .filter(e => e.datum.startsWith(monat.monat))
                .reduce((sum, e) => sum + (e.htProTag || 0), 0);
            const ntKwhGesamt = interpolierteDaten
                .filter(e => e.datum.startsWith(monat.monat))
                .reduce((sum, e) => sum + (e.ntProTag || 0), 0);
            ...
            monatsZusammenfassungen.push({
                ...
                htKostenGesamt: interpolierteDaten
                    .filter(e => e.datum.startsWith(monat.monat))
                    .reduce((sum, e) => sum + (e.htProTag || 0) * (tarif ? tarif.htPreis : 0), 0),
                ntKostenGesamt: interpolierteDaten
                    .filter(e => e.datum.startsWith(monat.monat))
                    .reduce((sum, e) => sum + (e.ntProTag || 0) * (tarif ? tarif.ntPreis : 0), 0),
    ...
    berechneDetail() {
        ...
        const ht = htStr === '' ? 0 : (parseFloat(htStr.replace(',', '.')) || 0);
        const nt = ntStr === '' ? 0 : (parseFloat(ntStr.replace(',', '.')) || 0);
        ...
        const datum = new Date(this.aktuellerTag);
        const tarif = this.getTarifFuerDatum(datum);
        ...
        // Wenn kein Tarif existiert, nur HT/NT anzeigen, keine Kostenberechnung
        if (!tarif) {
            ...
            if (vorherigerWert) {
                ...
                const tageDiff = this.tageZwischen(vorherigerWert.datum, this.aktuellerTag);
                
                if (tageDiff > 0) {
                    const htProTag = htDiff / tageDiff;
                    const ntProTag = ntDiff / tageDiff;
                    document.getElementById('detail-ht-tag').textContent = htProTag.toFixed(1).replace('.', ',') + ' kWh';
                    document.getElementById('detail-nt-tag').textContent = ntProTag.toFixed(1).replace('.', ',') + ' kWh';
                }
            }
        ...
        const vorherigerWert = this.getVorherigenMesswert(this.aktuellerTag);
        if (!vorherigerWert) {
            // Ohne vorherigen Wert nur Grundpreis anzeigen
            const grundpreisProTag = tarif.grundpreis / this.tageImMonat(datum);
            document.getElementById('detail-ht-tag').textContent = '0,0 kWh';
            document.getElementById('detail-nt-tag').textContent = '0,0 kWh';
            document.getElementById('detail-grundpreis').textContent = grundpreisProTag.toFixed(2).replace('.', ',') + ' €';
            document.getElementById('detail-ht-kosten').textContent = '0,00 €';
            document.getElementById('detail-nt-kosten').textContent = '0,00 €';
            document.getElementById('detail-gesamtkosten').textContent = grundpreisProTag.toFixed(2).replace('.', ',') + ' €';
            return;
        }
        ...
        if (tageDiff <= 0) return;

        const htProTag = htDiff / tageDiff;
        const ntProTag = ntDiff / tageDiff;
        const grundpreisProTag = tarif.grundpreis / this.tageImMonat(datum);

        const htKosten = htProTag * tarif.htPreis;
        const ntKosten = ntProTag * tarif.ntPreis;
        const gesamtkosten = htKosten + ntKosten + grundpreisProTag;

        document.getElementById('detail-ht-tag').textContent = htProTag.toFixed(1).replace('.', ',') + ' kWh';
        document.getElementById('detail-nt-tag').textContent = ntProTag.toFixed(1).replace('.', ',') + ' kWh';
        document.getElementById('detail-grundpreis').textContent = grundpreisProTag.toFixed(2).replace('.', ',') + ' €';
        document.getElementById('detail-ht-kosten').textContent = htKosten.toFixed(2).replace('.', ',') + ' €';
        document.getElementById('detail-nt-kosten').textContent = ntKosten.toFixed(2).replace('.', ',') + ' €';
        document.getElementById('detail-gesamtkosten').textContent = gesamtkosten.toFixed(2).replace('.', ',') + ' €';
    }
...
erstelleTarifElement(tarif) {
    ...
    speichereTarif() {
        ...
        const grundpreis = parseFloat(document.getElementById('tarif-grundpreis').value);
