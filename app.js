...
class StromrechnerApp {
    ...
    bindEvents() {
        ...
        // Jahresnavigation für Jahresübersicht
        document.getElementById('jahr-vorher').addEventListener('click', () => {
            this.wechsleJahr('vorher');
        });
        document.getElementById('jahr-naechster').addEventListener('click', () => {
            this.wechsleJahr('naechster');
        });
        document.getElementById('jahr-select').addEventListener('change', (e) => {
            this.aktuellesJahr = parseInt(e.target.value);
            this.zeigeJahresuebersicht();
        });

        // Prognose-Toggle
        document.getElementById('prognose-toggle').addEventListener('click', () => {
            const content = document.getElementById('prognose-content');
            const btn = document.getElementById('prognose-toggle');
            content.classList.toggle('active');
            btn.classList.toggle('active');
        });

        // Monatszusammenfassung-Toggle
        document.getElementById('monatszusammenfassung-toggle').addEventListener('click', () => {
            const content = document.getElementById('monatszusammenfassung-content');
            const btn = document.getElementById('monatszusammenfassung-toggle');
            content.classList.toggle('active');
            btn.classList.toggle('active');
        });
    ...
    wechsleTab(tabName) {
        ...
        // Jahresübersicht-Tab initialisieren
        if (tabName === 'jahresuebersicht') {
            this.initJahresuebersicht();
        }
    }
...
erstelleTarifElement(tarif) {
    ...
    initAuswertung() {
        ...
        ['verbrauch', 'kosten', 'temperatur', 'heizungen'].forEach(id => {
            document.getElementById(`toggle-${id}`).addEventListener('change', () => {
                // Radio-Groups ein-/ausblenden
                if (id === 'verbrauch') {
                    const radioGroup = document.getElementById('verbrauch-radio-group');
                    radioGroup.style.display = document.getElementById('toggle-verbrauch').checked ? 'flex' : 'none';
                }
                if (id === 'kosten') {
                    const radioGroup = document.getElementById('kosten-radio-group');
                    radioGroup.style.display = document.getElementById('toggle-kosten').checked ? 'flex' : 'none';
                }
                this.zeigeChart();
            });
        });
        ...
        document.querySelectorAll('input[name="verbrauch-mode"]').forEach(radio => {
            radio.addEventListener('change', () => {
                this.zeigeChart();
            });
        });
        document.querySelectorAll('input[name="kosten-mode"]').forEach(radio => {
            radio.addEventListener('change', () => {
                this.zeigeChart();
            });
        });
    ...
    aggregiereDaten(von, bis, modus) {
        ...
        // Interpolierte Daten berechnen (mit allen Werten im Zeitraum)
        const interpolierteDaten = this.berechneInterpolation(messwerteImZeitraum);

        if (modus === 'tag') {
            // Tägliche Werte - direkt aus interpolierten Daten
            interpolierteDaten.forEach(eintrag => {
                const zusatz = this.zusatzdaten[eintrag.datum] || {};
                daten.push({
                    datum: eintrag.datum,
                    htProTag: eintrag.htProTag || 0,
                    ntProTag: eintrag.ntProTag || 0,
                    verbrauch: (eintrag.htProTag || 0) + (eintrag.ntProTag || 0),
                    tageskosten: eintrag.tageskosten || 0,
            ...
            interpolierteDaten.forEach(eintrag => {
                ...
                wochen[wochenKey].htProTag += eintrag.htProTag || 0;
                wochen[wochenKey].ntProTag += eintrag.ntProTag || 0;
                wochen[wochenKey].verbrauch += (eintrag.htProTag || 0) + (eintrag.ntProTag || 0);
                wochen[wochenKey].tageskosten += eintrag.tageskosten || 0;
            ...
            const monate = {};
            interpolierteDaten.forEach(eintrag => {
                const monatKey = eintrag.datum.substring(0, 7); // YYYY-MM
                ...
                monate[monatKey].htProTag += eintrag.htProTag || 0;
                monate[monatKey].ntProTag += eintrag.ntProTag || 0;
                monate[monatKey].verbrauch += (eintrag.htProTag || 0) + (eintrag.ntProTag || 0);
                monate[monatKey].tageskosten += eintrag.tageskosten || 0;
    ...
    zeigeChart() {
        ...
        // Radio-Button-Werte auslesen
        const verbrauchMode = document.querySelector('input[name="verbrauch-mode"]:checked')?.value || 'gesamt';
        const kostenMode = document.querySelector('input[name="kosten-mode"]:checked')?.value || 'gesamt';
        ...
        daten.forEach(d => {
            if (zeigeVerbrauch) {
                let wert;
                if (verbrauchMode === 'ht') wert = d.htProTag;
                else if (verbrauchMode === 'nt') wert = d.ntProTag;
                else wert = d.verbrauch;
                
                if (wert !== null && wert !== undefined) {
                    minVerbrauch = Math.min(minVerbrauch, wert);
                    maxVerbrauch = Math.max(maxVerbrauch, wert);
                }
            }
            if (zeigeKosten && d.tageskosten !== null) {
                minKosten = Math.min(minKosten, d.tageskosten);
                maxKosten = Math.max(maxKosten, d.tageskosten);
            }
        ...
        if (zeigeVerbrauch && maxVerbrauch > -Infinity) {
            const verbrauchWerte = daten.map(d => {
                if (verbrauchMode === 'ht') return d.htProTag;
                else if (verbrauchMode === 'nt') return d.ntProTag;
                else return d.verbrauch;
            });
            drawLine(verbrauchWerte, 0, maxVerbrauch * 1.1, 'verbrauch');
        }
        if (zeigeKosten && maxKosten > -Infinity) {
            drawLine(daten.map(d => d.tageskosten), 0, maxKosten * 1.1, 'kosten');
        }
    ...
    zeigeTooltip(event, daten, reihe) {
        ...
        switch(reihe) {
            case 'verbrauch':
                reiheName = 'Verbrauch';
                if (verbrauchMode === 'ht') {
                    wert = daten.htProTag !== null ? daten.htProTag.toFixed(1) : '-';
                    reiheName = 'HT-Verbrauch';
                } else if (verbrauchMode === 'nt') {
                    wert = daten.ntProTag !== null ? daten.ntProTag.toFixed(1) : '-';
                    reiheName = 'NT-Verbrauch';
                } else {
                    wert = daten.verbrauch !== null ? daten.verbrauch.toFixed(1) : '-';
                }
                einheit = 'kWh';
                break;
            case 'kosten':
                reiheName = 'Kosten';
                wert = daten.tageskosten !== null ? daten.tageskosten.toFixed(2) : '-';
                einheit = '€';
                break;
            case 'temperatur':
    ...
    // Jahresübersicht initialisieren
    initJahresuebersicht() {
        this.initialisiereJahrSelect();
        this.zeigeJahresuebersicht();
    }

    // Jahr-Select initialisieren
    initialisiereJahrSelect() {
        ...
        jahrSelect.value = this.aktuellesJahr;
    }

    // Jahr wechseln
    wechsleJahr(richtung) {
        if (richtung === 'vorher') {
            this.aktuellesJahr -= 1;
        } else {
            this.aktuellesJahr += 1;
        }

        // Jahr-Select aktualisieren
        document.getElementById('jahr-select').value = this.aktuellesJahr;
        this.zeigeJahresuebersicht();
    }

    // Jahresübersicht anzeigen
    zeigeJahresuebersicht() {
        // Jahr-Select aktualisieren
        document.getElementById('jahr-select').value = this.aktuellesJahr;

        // Jahresdaten berechnen
        const jahresDaten = this.berechneJahresDaten(this.aktuellesJahr);

        // Hauptkennzahlen anzeigen
        this.zeigeHauptkennzahlen(jahresDaten);

        // Jahresvergleich anzeigen
        this.zeigeJahresvergleich(jahresDaten);

        // Extremwerte anzeigen
        this.zeigeExtremwerte(jahresDaten);

        // Prognose berechnen und anzeigen
        this.zeigePrognose(jahresDaten);

        // Datenqualität anzeigen
        this.zeigeDatenqualitaet(jahresDaten);

        // Monatszusammenfassung anzeigen
        this.zeigeMonatszusammenfassung(jahresDaten);
    }

    // Jahresdaten berechnen
    berechneJahresDaten(jahr) {
        const interpolierteDaten = this.berechneInterpolation([...this.messwerte].sort((a, b) => new Date(a.datum) - new Date(b.datum)));

        // Daten für das Jahr filtern
        const jahrDaten = interpolierteDaten.filter(eintrag => eintrag.datum.startsWith(jahr + '-'));

        // Monatsdaten berechnen
        const monatsDaten = {};
        for (let monat = 0; monat < 12; monat++) {
            const monatKey = `${jahr}-${String(monat + 1).padStart(2, '0')}`;
            monatsDaten[monatKey] = jahrDaten.filter(eintrag => eintrag.datum.startsWith(monatKey));
        }
        ...
        let gesamtNT = 0;
        let gesamtKosten = 0;
        let gesamtHTKosten = 0;
        let gesamtNTKosten = 0;
        let gesamtGrundkosten = 0;
        let tageMitDaten = 0;
        let interpolierteTage = 0;

        jahrDaten.forEach(eintrag => {
            gesamtVerbrauch += eintrag.tagesverbrauch;
            gesamtKosten += eintrag.tageskosten;
            if (eintrag.interpoliert) {
                interpolierteTage++;
            } else {
                tageMitDaten++;
        ...
        // HT/NT und Kosten berechnen
        Object.values(monatsDaten).forEach(monatEintraege => {
            monatEintraege.forEach(eintrag => {
                const kosten = this.berechneKosten(eintrag.datum, eintrag.ht, eintrag.nt);
                if (kosten.htProTag > 0) {
                    gesamtHT += kosten.htProTag;
                    gesamtHTKosten += kosten.htKosten;
                }
                if (kosten.ntProTag > 0) {
                    gesamtNT += kosten.ntProTag;
                    gesamtNTKosten += kosten.ntKosten;
                }
                if (kosten.grundpreisProTag > 0) {
                    gesamtGrundkosten += kosten.grundpreisProTag;
                }
            });
        });

        // Durchschnittswerte berechnen
        const tageImJahr = jahrDaten.length > 0 ? jahrDaten.length : 365;
        const durchschnittVerbrauchMonat = gesamtVerbrauch / 12;
        const durchschnittKostenMonat = gesamtKosten / 12;
        const durchschnittVerbrauchTag = gesamtVerbrauch / tageImJahr;
        const durchschnittKostenTag = gesamtKosten / tageImJahr;

        // Zeitraum der Daten
        const zeitraum = jahrDaten.length > 0 ? {
            von: jahrDaten[0].datum,
            bis: jahrDaten[jahrDaten.length - 1].datum
        } : null;
        ...
        return {
            ...
            gesamtNTKosten,
            gesamtGrundkosten,
            durchschnittVerbrauchMonat,
            durchschnittKostenMonat,
            durchschnittVerbrauchTag,
            durchschnittKostenTag,
            monatsDaten,
            tageMitDaten,
            interpolierteTage,
            zeitraum
    ...
    // Hauptkennzahlen anzeigen
    zeigeHauptkennzahlen(jahresDaten) {
        document.getElementById('jahres-verbrauch-gesamt').textContent = jahresDaten.gesamtVerbrauch.toFixed(1).replace('.', ',');
        document.getElementById('jahres-verbrauch-ht').textContent = jahresDaten.gesamtHT.toFixed(1).replace('.', ',');
        document.getElementById('jahres-verbrauch-nt').textContent = jahresDaten.gesamtNT.toFixed(1).replace('.', ',');
        document.getElementById('jahres-kosten-gesamt').textContent = jahresDaten.gesamtKosten.toFixed(2).replace('.', ',');
        document.getElementById('jahres-kosten-ht').textContent = jahresDaten.gesamtHTKosten.toFixed(2).replace('.', ',');
        document.getElementById('jahres-kosten-nt').textContent = jahresDaten.gesamtNTKosten.toFixed(2).replace('.', ',');
        document.getElementById('jahres-kosten-grund').textContent = jahresDaten.gesamtGrundkosten.toFixed(2).replace('.', ',');
        document.getElementById('durchschnitt-verbrauch-monat').textContent = jahresDaten.durchschnittVerbrauchMonat.toFixed(1).replace('.', ',');
        document.getElementById('durchschnitt-kosten-monat').textContent = jahresDaten.durchschnittKostenMonat.toFixed(2).replace('.', ',');
        document.getElementById('durchschnitt-verbrauch-tag').textContent = jahresDaten.durchschnittVerbrauchTag.toFixed(1).replace('.', ',');
        document.getElementById('durchschnitt-kosten-tag').textContent = jahresDaten.durchschnittKostenTag.toFixed(2).replace('.', ',');
    }

    // Jahresvergleich anzeigen
    zeigeJahresvergleich(jahresDaten) {
        const vorjahrDaten = this.berechneJahresDaten(jahresDaten.jahr - 1);
        const vergleichBereich = document.getElementById('jahresvergleich-bereich');

        if (vorjahrDaten.gesamtVerbrauch === 0) {
            vergleichBereich.style.display = 'none';
            return;
        }

        vergleichBereich.style.display = 'block';

        // Vorjahr anzeigen
        document.getElementById('vergleich-vorjahr-jahr').textContent = jahresDaten.jahr - 1;

        // Verbrauch Änderung
        const verbrauchDiff = jahresDaten.gesamtVerbrauch - vorjahrDaten.gesamtVerbrauch;
        const verbrauchProz = (verbrauchDiff / vorjahrDaten.gesamtVerbrauch) * 100;

        document.getElementById('vergleich-verbrauch-abs').textContent = (verbrauchDiff >= 0 ? '+' : '') + verbrauchDiff.toFixed(1).replace('.', ',') + ' kWh';
        document.getElementById('vergleich-verbrauch-proz').textContent = (verbrauchProz >= 0 ? '+' : '') + verbrauchProz.toFixed(1).replace('.', ',') + '%';

        // Kosten Änderung
        const kostenDiff = jahresDaten.gesamtKosten - vorjahrDaten.gesamtKosten;
        const kostenProz = (kostenDiff / vorjahrDaten.gesamtKosten) * 100;

        document.getElementById('vergleich-kosten-abs').textContent = (kostenDiff >= 0 ? '+' : '') + kostenDiff.toFixed(2).replace('.', ',') + ' €';
        document.getElementById('vergleich-kosten-proz').textContent = (kostenProz >= 0 ? '+' : '') + kostenProz.toFixed(1).replace('.', ',') + '%';
    }

    // Extremwerte anzeigen
    zeigeExtremwerte(jahresDaten) {
        let maxVerbrauchMonat = null;
        let minVerbrauchMonat = null;
        let maxKostenMonat = null;
        let minKostenMonat = null;

        const aktuellesDatum = new Date();
        const aktuellerMonat = aktuellesDatum.toISOString().substring(0, 7);

        Object.entries(jahresDaten.monatsDaten).forEach(([monatKey, eintraege]) => {
            if (eintraege.length === 0) return;

            // Laufenden Monat ausschließen (nur abgeschlossene Monate)
            if (monatKey === aktuellerMonat && jahresDaten.jahr === aktuellesDatum.getFullYear()) {
                return;
            }

            const monatVerbrauch = eintraege.reduce((sum, e) => sum + e.tagesverbrauch, 0);
            const monatKosten = eintraege.reduce((sum, e) => sum + e.tageskosten, 0);

            const monatName = new Date(monatKey + '-01').toLocaleDateString('de-DE', { month: 'long', year: 'numeric' });

            if (!maxVerbrauchMonat || monatVerbrauch > maxVerbrauchMonat.verbrauch) {
                maxVerbrauchMonat = { name: monatName, verbrauch: monatVerbrauch };
            }
            if (!minVerbrauchMonat || monatVerbrauch < minVerbrauchMonat.verbrauch) {
                minVerbrauchMonat = { name: monatName, verbrauch: monatVerbrauch };
            }
            if (!maxKostenMonat || monatKosten > maxKostenMonat.kosten) {
                maxKostenMonat = { name: monatName, kosten: monatKosten };
            }
            if (!minKostenMonat || monatKosten < minKostenMonat.kosten) {
                minKostenMonat = { name: monatName, kosten: monatKosten };
            }
        });

        document.getElementById('extrem-verbrauch-max').textContent = maxVerbrauchMonat ? `${maxVerbrauchMonat.name} (${maxVerbrauchMonat.verbrauch.toFixed(1).replace('.', ',')} kWh)` : '-';
        document.getElementById('extrem-verbrauch-min').textContent = minVerbrauchMonat ? `${minVerbrauchMonat.name} (${minVerbrauchMonat.verbrauch.toFixed(1).replace('.', ',')} kWh)` : '-';
        document.getElementById('extrem-kosten-max').textContent = maxKostenMonat ? `${maxKostenMonat.name} (${maxKostenMonat.kosten.toFixed(2).replace('.', ',')} €)` : '-';
        document.getElementById('extrem-kosten-min').textContent = minKostenMonat ? `${minKostenMonat.name} (${minKostenMonat.kosten.toFixed(2).replace('.', ',')} €)` : '-';
    }

    // Prognose anzeigen
    zeigePrognose(jahresDaten) {
        const prognoseBereich = document.querySelector('.prognose-bereich');
        const prognoseHinweis = document.getElementById('prognose-hinweis');
        const prognoseWerte = document.getElementById('prognose-werte');

        const aktuellesJahr = new Date().getFullYear();

        // Prognose nur bei laufenden Jahren anzeigen
        if (jahresDaten.jahr !== aktuellesJahr) {
            prognoseBereich.style.display = 'none';
            return;
        }

        // Prüfen ob Jahr Daten hat aber nicht bis zum Ende
        const maxTage = this.istSchaltjahr(jahresDaten.jahr) ? 366 : 365;
        const gesamtTage = jahresDaten.tageMitDaten + jahresDaten.interpolierteTage;

        if (gesamtTage >= maxTage) {
            // Jahr ist abgeschlossen, Prognose ausblenden
            prognoseBereich.style.display = 'none';
            return;
        }

        prognoseBereich.style.display = 'block';

        // Prüfen ob mindestens 3 Monate Daten vorhanden
        const monateMitDaten = Object.values(jahresDaten.monatsDaten).filter(eintraege => eintraege.length > 0).length;

        if (monateMitDaten < 3) {
            prognoseHinweis.style.display = 'block';
            prognoseWerte.style.display = 'none';
            return;
        }

        prognoseHinweis.style.display = 'none';
        prognoseWerte.style.display = 'grid';

        // Exakte Prognose durch Summation aller Monatsprognosen des laufenden Jahres berechnen
        let prognoseVerbrauchGesamt = 0;
        let prognoseKostenGesamt = 0;
        let echtVerbrauchGesamt = 0;
        let echtKostenGesamt = 0;

        for (let monat = 0; monat < 12; monat++) {
            const monatKey = `${jahresDaten.jahr}-${String(monat + 1).padStart(2, '0')}`;
            const eintraege = jahresDaten.monatsDaten[monatKey] || [];
            
            // Echte Werte für diesen Monat aufsummieren
            eintraege.forEach(e => {
                echtVerbrauchGesamt += e.tagesverbrauch;
                echtKostenGesamt += e.tageskosten;
            });

            // Monatsprognose berechnen
            const monatsProg = this.berechneMonatsPrognose(monatKey, jahresDaten.jahr, eintraege);
            prognoseVerbrauchGesamt += monatsProg.verbrauch;
            prognoseKostenGesamt += monatsProg.kosten;
        }

        const prognoseVerbrauchNur = Math.max(0, prognoseVerbrauchGesamt - echtVerbrauchGesamt);
        const prognoseKostenNur = Math.max(0, prognoseKostenGesamt - echtKostenGesamt);

        document.getElementById('prognose-verbrauch-gesamt').textContent = prognoseVerbrauchGesamt.toFixed(1).replace('.', ',');
        document.getElementById('prognose-verbrauch-echt').textContent = echtVerbrauchGesamt.toFixed(1).replace('.', ',');
        document.getElementById('prognose-verbrauch-prognose').textContent = prognoseVerbrauchNur.toFixed(1).replace('.', ',');
        document.getElementById('prognose-kosten-gesamt').textContent = prognoseKostenGesamt.toFixed(2).replace('.', ',');
        document.getElementById('prognose-kosten-echt').textContent = echtKostenGesamt.toFixed(2).replace('.', ',');
        document.getElementById('prognose-kosten-prognose').textContent = prognoseKostenNur.toFixed(2).replace('.', ',');

        // Prognostizierte Durchschnitte berechnen (Monat / Tag)
        const maxTageJahr = this.istSchaltjahr(jahresDaten.jahr) ? 366 : 365;
        const progDurchschnittVerbrauchMonat = prognoseVerbrauchGesamt / 12;
        const progDurchschnittKostenMonat = prognoseKostenGesamt / 12;
        const progDurchschnittVerbrauchTag = prognoseVerbrauchGesamt / maxTageJahr;
        const progDurchschnittKostenTag = prognoseKostenGesamt / maxTageJahr;

        document.getElementById('prognose-durchschnitt-verbrauch-monat').textContent = progDurchschnittVerbrauchMonat.toFixed(1).replace('.', ',');
        document.getElementById('prognose-durchschnitt-kosten-monat').textContent = progDurchschnittKostenMonat.toFixed(2).replace('.', ',');
        document.getElementById('prognose-durchschnitt-verbrauch-tag').textContent = progDurchschnittVerbrauchTag.toFixed(1).replace('.', ',');
        document.getElementById('prognose-durchschnitt-kosten-tag').textContent = progDurchschnittKostenTag.toFixed(2).replace('.', ',');

        // Abweichung zum Vorjahr
        const vorjahrDaten = this.berechneJahresDaten(jahresDaten.jahr - 1);
        if (vorjahrDaten.gesamtVerbrauch > 0) {
            const verbrauchDiff = prognoseVerbrauchGesamt - vorjahrDaten.gesamtVerbrauch;
            const verbrauchProz = (verbrauchDiff / vorjahrDaten.gesamtVerbrauch) * 100;
            const kostenDiff = prognoseKostenGesamt - vorjahrDaten.gesamtKosten;
            const kostenProz = (kostenDiff / vorjahrDaten.gesamtKosten) * 100;

            document.getElementById('prognose-abweichung-verbrauch-abs').textContent = (verbrauchDiff >= 0 ? '+' : '') + verbrauchDiff.toFixed(1).replace('.', ',') + ' kWh';
            document.getElementById('prognose-abweichung-verbrauch-proz').textContent = (verbrauchProz >= 0 ? '+' : '') + verbrauchProz.toFixed(1).replace('.', ',') + '%';
            document.getElementById('prognose-abweichung-kosten-abs').textContent = (kostenDiff >= 0 ? '+' : '') + kostenDiff.toFixed(2).replace('.', ',') + ' €';
            document.getElementById('prognose-abweichung-kosten-proz').textContent = (kostenProz >= 0 ? '+' : '') + kostenProz.toFixed(1).replace('.', ',') + '%';
        } else {
            document.getElementById('prognose-abweichung-verbrauch-abs').textContent = '-';
            document.getElementById('prognose-abweichung-verbrauch-proz').textContent = '-';
            document.getElementById('prognose-abweichung-kosten-abs').textContent = '-';
            document.getElementById('prognose-abweichung-kosten-proz').textContent = '-';
        }
    }

    // Datenqualität anzeigen
    zeigeDatenqualitaet(jahresDaten) {
        const gesamtTage = jahresDaten.tageMitDaten + jahresDaten.interpolierteTage;
        const maxTage = this.istSchaltjahr(jahresDaten.jahr) ? 366 : 365;
        const prozent = (gesamtTage / maxTage) * 100;

        document.getElementById('datenqualitaet-gesamt').textContent = gesamtTage;
        document.getElementById('datenqualitaet-max-tage').textContent = maxTage;
        document.getElementById('datenqualitaet-prozent').textContent = prozent.toFixed(1).replace('.', ',');
        document.getElementById('datenqualitaet-gespeichert').textContent = jahresDaten.tageMitDaten;
        document.getElementById('datenqualitaet-interpoliert').textContent = jahresDaten.interpolierteTage;

        if (jahresDaten.zeitraum) {
            const vonDatum = new Date(jahresDaten.zeitraum.von);
            const bisDatum = new Date(jahresDaten.zeitraum.bis);
            const vonFormatiert = this.formatiereDatum(jahresDaten.zeitraum.von);
            const bisFormatiert = this.formatiereDatum(jahresDaten.zeitraum.bis);
            document.getElementById('datenqualitaet-zeitraum').textContent = `${vonFormatiert} - ${bisFormatiert}`;
        } else {
            document.getElementById('datenqualitaet-zeitraum').textContent = '-';
        }
    }
    ...
    // Monatszusammenfassung anzeigen
    zeigeMonatszusammenfassung(jahresDaten) {
        const tbody = document.getElementById('monatszusammenfassung-body');
        tbody.innerHTML = '';

        const monatNamen = ['Januar', 'Februar', 'März', 'April', 'Mai', 'Juni', 'Juli', 'August', 'September', 'Oktober', 'November', 'Dezember'];
        const aktuellesJahr = new Date().getFullYear();
        const aktuellerMonatKey = new Date().toISOString().substring(0, 7);
        const istLaufendesJahr = jahresDaten.jahr === aktuellesJahr;

        // Alle 12 Monate durchgehen
        for (let monat = 0; monat < 12; monat++) {
            const monatKey = `${jahresDaten.jahr}-${String(monat + 1).padStart(2, '0')}`;
            const monatName = monatNamen[monat];
            const eintraege = jahresDaten.monatsDaten[monatKey] || [];

            if (eintraege.length === 0) {
                // Monat ohne Daten
                if (istLaufendesJahr && monatKey >= aktuellerMonatKey) {
                    // Fehlender/Zukünftiger Monat im laufenden Jahr: Prognose
                    const prognose = this.berechneMonatsPrognose(monatKey, jahresDaten.jahr, []);
                    const row = document.createElement('tr');
                    row.classList.add('prognostiziert');
                    row.innerHTML = `
                        <td>${monatName} (P)</td>
                        <td>${prognose.ht.toFixed(1).replace('.', ',')}</td>
                        <td>${prognose.nt.toFixed(1).replace('.', ',')}</td>
                        <td>${prognose.verbrauch.toFixed(1).replace('.', ',')}</td>
                        <td>${prognose.kosten.toFixed(2).replace('.', ',')}</td>
                        <td>${prognose.htKosten.toFixed(2).replace('.', ',')}</td>
                        <td>${prognose.ntKosten.toFixed(2).replace('.', ',')}</td>
                        <td>${prognose.grundkosten.toFixed(2).replace('.', ',')}</td>
                        <td>${prognose.durchschnittVerbrauchTag.toFixed(1).replace('.', ',')}</td>
                        <td>${prognose.durchschnittKostenTag.toFixed(2).replace('.', ',')}</td>
                        <td>-</td>
                        <td>-</td>
                    `;
                    tbody.appendChild(row);
                } else {
                    // Monat ohne Daten in vergangenen Jahren oder vergangenen Monaten
                    const row = document.createElement('tr');
                    row.innerHTML = `
                        <td>${monatName}</td>
                        <td>-</td>
            ...
            // Monat mit Daten - echte Werte berechnen
            let monatHT = 0;
            let monatNT = 0;
            let monatVerbrauch = 0;
            let monatKosten = 0;
            let monatHTKosten = 0;
            let monatNTKosten = 0;
            let monatGrundkosten = 0;
            let monatHeizung = 0;
            let monatTemperatur = 0;
            let tageMitHeizung = 0;
            let tageMitTemperatur = 0;

            eintraege.forEach(eintrag => {
                monatVerbrauch += eintrag.tagesverbrauch;
                monatKosten += eintrag.tageskosten;

                const kosten = this.berechneKosten(eintrag.datum, eintrag.ht, eintrag.nt);
                if (kosten.htProTag > 0) monatHT += kosten.htProTag;
                if (kosten.ntProTag > 0) monatNT += kosten.ntProTag;
                if (kosten.htKosten > 0) monatHTKosten += kosten.htKosten;
                if (kosten.ntKosten > 0) monatNTKosten += kosten.ntKosten;
                if (kosten.grundpreisProTag > 0) monatGrundkosten += kosten.grundpreisProTag;

                const zusatz = this.zusatzdaten[eintrag.datum];
                if (zusatz) {
                    if (zusatz.heizungen !== undefined && zusatz.heizungen !== null) {
                        monatHeizung += zusatz.heizungen;
                        tageMitHeizung++;
                    }
                    if (zusatz.temperatur !== undefined && zusatz.temperatur !== null) {
                        monatTemperatur += zusatz.temperatur;
                        tageMitTemperatur++;
                    }
                }
            });

            const tageImMonat = eintraege.length;
            const durchschnittVerbrauchTag = tageImMonat > 0 ? monatVerbrauch / tageImMonat : 0;
            const durchschnittKostenTag = tageImMonat > 0 ? monatKosten / tageImMonat : 0;
            const durchschnittHeizung = tageMitHeizung > 0 ? monatHeizung / tageMitHeizung : null;
            const durchschnittTemperatur = tageMitTemperatur > 0 ? monatTemperatur / tageMitTemperatur : null;

            // Zeile mit echten Bisher-Daten
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${monatName}</td>
                <td>${monatHT.toFixed(1).replace('.', ',')}</td>
                <td>${monatNT.toFixed(1).replace('.', ',')}</td>
                <td>${monatVerbrauch.toFixed(1).replace('.', ',')}</td>
                <td>${monatKosten.toFixed(2).replace('.', ',')}</td>
                <td>${monatHTKosten.toFixed(2).replace('.', ',')}</td>
                <td>${monatNTKosten.toFixed(2).replace('.', ',')}</td>
                <td>${monatGrundkosten.toFixed(2).replace('.', ',')}</td>
                <td>${durchschnittVerbrauchTag.toFixed(1).replace('.', ',')}</td>
                <td>${durchschnittKostenTag.toFixed(2).replace('.', ',')}</td>
                <td>${durchschnittHeizung !== null ? durchschnittHeizung.toFixed(1).replace('.', ',') : '-'}</td>
                <td>${durchschnittTemperatur !== null ? durchschnittTemperatur.toFixed(1).replace('.', ',') : '-'}</td>
            `;
            tbody.appendChild(row);

            // Laufender Monat im laufenden Jahr: zusätzliche Zeile mit Gesamtmotatsprognose (P)
            if (istLaufendesJahr && monatKey === aktuellerMonatKey) {
                const prognose = this.berechneMonatsPrognose(monatKey, jahresDaten.jahr, eintraege);
                const prognoseRow = document.createElement('tr');
                prognoseRow.classList.add('prognostiziert');
                prognoseRow.innerHTML = `
                    <td>${monatName} (P)</td>
                    <td>${prognose.ht.toFixed(1).replace('.', ',')}</td>
                    <td>${prognose.nt.toFixed(1).replace('.', ',')}</td>
                    <td>${prognose.verbrauch.toFixed(1).replace('.', ',')}</td>
                    ...
                    <td>-</td>
                `;
                tbody.appendChild(prognoseRow);
            }
        }
    }

    // Monatsprognose berechnen
    berechneMonatsPrognose(monatKey, zielJahr, monatsEintraege = []) {
        const datumObj = new Date(monatKey + '-01');
        const tageImMonat = this.tageImMonat(datumObj);
        const echteTage = monatsEintraege.length;
        ...
        let echtHTKosten = 0;
        let echtNTKosten = 0;
        let echtGrundkosten = 0;

        monatsEintraege.forEach(eintrag => {
            echtVerbrauch += eintrag.tagesverbrauch;
            echtKosten += eintrag.tageskosten;
            const kosten = this.berechneKosten(eintrag.datum, eintrag.ht, eintrag.nt);
            if (kosten.htProTag > 0) echtHT += kosten.htProTag;
            if (kosten.ntProTag > 0) echtNT += kosten.ntProTag;
            if (kosten.htKosten > 0) echtHTKosten += kosten.htKosten;
            if (kosten.ntKosten > 0) echtNTKosten += kosten.ntKosten;
            if (kosten.grundpreisProTag > 0) echtGrundkosten += kosten.grundpreisProTag;
        });

        // Wenn der Monat bereits vollständig erfasst ist
        if (echteTage >= tageImMonat) {
            return {
                ht: echtHT,
                nt: echtNT,
                verbrauch: echtVerbrauch,
                kosten: echtKosten,
                htKosten: echtHTKosten,
                ntKosten: echtNTKosten,
                grundkosten: echtGrundkosten,
                durchschnittVerbrauchTag: echtVerbrauch / tageImMonat,
                durchschnittKostenTag: echtKosten / tageImMonat
            };
        }

        const verbleibendeTage = tageImMonat - echteTage;

        // Vorjahresdaten für denselben Monat überprüfen
        const vorjahrMonatKey = `${zielJahr - 1}-${monatKey.substring(5, 7)}`;
        const vorjahrDaten = this.berechneJahresDaten(zielJahr - 1);
        const vorjahrEintraege = vorjahrDaten.monatsDaten[vorjahrMonatKey] || [];

        let htProTagPrognose = 0;
        let ntProTagPrognose = 0;

        if (vorjahrEintraege.length > 0) {
            // Vorjahresdaten vorhanden -> Tagesdurchschnitt aus dem Vorjahresmonat nutzen
            let vorjahrHT = 0;
            let vorjahrNT = 0;
            vorjahrEintraege.forEach(eintrag => {
                const kosten = this.berechneKosten(eintrag.datum, eintrag.ht, eintrag.nt);
                if (kosten.htProTag > 0) vorjahrHT += kosten.htProTag;
                if (kosten.ntProTag > 0) vorjahrNT += kosten.ntProTag;
            });
            const vorjahrTage = vorjahrEintraege.length;
            htProTagPrognose = vorjahrHT / vorjahrTage;
            ntProTagPrognose = vorjahrNT / vorjahrTage;
        } else {
            // Fallback: Keine Vorjahresdaten -> bisherigen Tagesdurchschnitt des aktuellen Jahres nutzen
            const interpolierteDaten = this.berechneInterpolation([...this.messwerte].sort((a, b) => new Date(a.datum) - new Date(b.datum)));
            const jahrDaten = interpolierteDaten.filter(eintrag => eintrag.datum.startsWith(zielJahr + '-'));

            let gesamtHT = 0;
            let gesamtNT = 0;
            const anzahlTage = jahrDaten.length;

            if (anzahlTage > 0) {
                jahrDaten.forEach(e => {
                    const kosten = this.berechneKosten(e.datum, e.ht, e.nt);
                    if (kosten.htProTag > 0) gesamtHT += kosten.htProTag;
                    if (kosten.ntProTag > 0) gesamtNT += kosten.ntProTag;
                });
                htProTagPrognose = gesamtHT / anzahlTage;
                ntProTagPrognose = gesamtNT / anzahlTage;
            } else if (interpolierteDaten.length > 0) {
                // Fallback auf alle verfügbaren Messdaten, falls im aktuellen Jahr noch nichts vorliegt
                interpolierteDaten.forEach(e => {
                    const kosten = this.berechneKosten(e.datum, e.ht, e.nt);
                    if (kosten.htProTag > 0) gesamtHT += kosten.htProTag;
                    if (kosten.ntProTag > 0) gesamtNT += kosten.ntProTag;
                });
                htProTagPrognose = gesamtHT / interpolierteDaten.length;
                ntProTagPrognose = gesamtNT / interpolierteDaten.length;
            }
        }

        // Prognose für die verbleibenden Tage berechnen
        const prognoseHTNur = htProTagPrognose * verbleibendeTage;
        const prognoseNTNur = ntProTagPrognose * verbleibendeTage;
        const prognoseVerbrauchNur = prognoseHTNur + prognoseNTNur;

        // Tarif für den Prognosezeitraum ermitteln
        const tarif = this.getTarifFuerDatum(datumObj);
        const grundpreisProTag = tarif ? (tarif.grundpreis / tageImMonat) : 0;
        const htPreis = tarif ? tarif.htPreis : 0;
        const ntPreis = tarif ? tarif.ntPreis : 0;

        const prognoseHTKostenNur = prognoseHTNur * htPreis;
        const prognoseNTKostenNur = prognoseNTNur * ntPreis;
        const prognoseGrundkostenNur = verbleibendeTage * grundpreisProTag;
        const prognoseKostenNur = prognoseHTKostenNur + prognoseNTKostenNur + prognoseGrundkostenNur;

        // Gesamtsumme = Echt (bisher) + Prognose (verbleibend)
        const totalHT = echtHT + prognoseHTNur;
        const totalNT = echtNT + prognoseNTNur;
        const totalVerbrauch = echtVerbrauch + prognoseVerbrauchNur;
        const totalHTKosten = echtHTKosten + prognoseHTKostenNur;
        const totalNTKosten = echtNTKosten + prognoseNTKostenNur;
        const totalGrundkosten = echtGrundkosten + prognoseGrundkostenNur;
        const totalKosten = echtKosten + prognoseKostenNur;

        return {
            ht: totalHT,
            nt: totalNT,
            verbrauch: totalVerbrauch,
            kosten: totalKosten,
            htKosten: totalHTKosten,
            ntKosten: totalNTKosten,
            grundkosten: totalGrundkosten,
            durchschnittVerbrauchTag: totalVerbrauch / tageImMonat,
            durchschnittKostenTag: totalKosten / tageImMonat
        };
    }
}
